...
export function WilhelmAvatar({ size = 44, variant = 'ink', style = {} }) {
  const bg = {
    ink:   '#ede4d3',
    kraft: '#c9b896',
    bone:  '#2a3128',
...
export function Stamp({ children, color = '#b85a3c', rotate = -3 }) {
  return (
    <div style={{
      display: 'inline-flex', alignItems: 'center', gap: 6,
      fontFamily: 'JetBrains Mono, monospace', fontSize: 9, letterSpacing: '0.18em',
      textTransform: 'uppercase', color, padding: '5px 10px',
      border: `2px solid ${color}`, borderRadius: 3,
      transform: `rotate(${rotate}deg)`, background: 'rgba(217,119,87,0.04)',
    }}>{children}</div>
  );
}
