    ...
    <>
      <main style={{ paddingTop: 24 }}>
          ...
          <div className="flex between center">
            ...
            <div className="mono" style={{ fontSize: 10, letterSpacing: '.16em' }}>
              <span style={{
                ...
                background: '#5b6b5a', marginRight: 6,
              }}/>
              LIVE · 11:24
            </div>
          </div>
          <h1 style={{ marginTop: 14, fontSize: 28, lineHeight: 1, letterSpacing: '-0.02em', fontWeight: 400 }}>
            Du er <em>2,4 km</em><br/>fra Roenshaugen.
          </h1>
        ...
        <div style={{ margin: '18px 18px 0', borderRadius: 4, overflow: 'hidden',
          border: '1px solid rgba(26,31,26,.22)', position: 'relative' }}>
          <RouteMap width={324} height={180} theme="paper"/>
          <div style={{ position: 'absolute', left: '62%', top: '42%' }}>
            <div style={{
              width: 14, height: 14, borderRadius: '50%', background: 'var(--ember)',
              border: '3px solid #ede4d3', boxShadow: '0 0 0 4px rgba(217,119,87,.25)',
            }}/>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, padding: '14px 18px 0' }}>
          {[
            ...
            <div key={c.l} style={{
              background: '#fff8ea', border: '1px solid rgba(26,31,26,.18)',
              borderRadius: 4, padding: '10px 12px',
            }}>
              <div className="mono" style={{ fontSize: 9, letterSpacing: '.18em', opacity: .55 }}>{c.l}</div>
              <div style={{ fontSize: 15, marginTop: 4, lineHeight: 1.1 }}>{c.v}</div>
              <div className="mono" style={{ fontSize: 9, opacity: .55, marginTop: 3 }}>{c.s}</div>
            </div>
          ))}
        </div>

        <section style={{
          margin: '14px 18px 0', padding: '12px 14px', borderRadius: 4,
          border: '1px dashed rgba(184,90,60,.55)', background: 'rgba(217,119,87,0.06)',
          display: 'flex', gap: 10,
        }}>
          ...
          <div>
            ...
            <p className="hand" style={{ fontSize: 18, color: '#1a3a5a', marginTop: 2, lineHeight: 1.15, marginBottom: 0 }}>
              Yret setter seg ved middag. Press det neste kilometeret, så tar du teen din under grana ved 9,5.
            </p>
          </div>
        </section>

        <div style={{ margin: '14px 18px 24px', display: 'flex', gap: 8 }}>
          <button type="button" className="btn-ghost ui" style={{ flex: 1 }}>
            <Glyph name="pin" size={16}/> Marker sted
          </button>
          <button type="button" className="btn-ember" style={{ background: '#1a1f1a', flex: '0 0 auto' }}>
