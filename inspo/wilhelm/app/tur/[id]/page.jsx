...
const DETAILS = {
  'dag-1': {
    pos: 'I / III', title: 'Røvollen til Linneset',
    ...
    cabin: { name: 'Linneset', coord: '60,9322 N · 12,3812 Ø · 4 KØYER · GASS',
             note: 'Vinduet ut mot vannet sitter litt skjevt — løft før du lukker.' },
  },
  'dag-2': {
    pos: 'II / III', title: 'Linneset til Roenshaugen',
    ...
    cabin: { name: 'Roenshaugen', coord: '60,9871 N · 12,4502 Ø · 6 KØYER · VED',
             note: 'Ovnen ryker når du fyrer kald. Tenn opp med opptenningsved først, slik jeg viste deg.' },
  },
  'dag-3': {
    pos: 'III / III', title: 'Roenshaugen til Røvollen',
    ...
    cabin: { name: 'Røvollen', coord: '60,8911 N · 12,3120 Ø · BIL · BUSSHOLDEPLASS',
      ...
      <main style={{ paddingTop: 24 }}>
        <div style={{ padding: '16px 22px 0' }}>
          <div className="flex between center">
            <Link href="/tur" className="pill" style={{ textDecoration: 'none' }}>← OVERSIKT</Link>
            <div className="mono" style={{ fontSize: 10, letterSpacing: '.18em', opacity: .6 }}>DAG {d.pos}</div>
          </div>
        ...
        <div style={{ margin: '18px 18px 0', padding: '12px 14px', background: '#fff8ea',
          border: '1px solid rgba(26,31,26,.2)', borderRadius: 4 }}>
          <div className="mono" style={{ fontSize: 9, letterSpacing: '.18em', opacity: .6, marginBottom: 6 }}>HØYDE · M</div>
          <Elevation width={290} height={70}/>
          <div className="flex between mono" style={{ fontSize: 9, opacity: .6, marginTop: 4 }}>
            <span>0 KM</span><span>{Math.round(parseInt(d.stats) / 2)}</span><span>{parseInt(d.stats)} KM</span>
          </div>
        </div>

        <div style={{ padding: '18px 22px 0' }}>
          <div className="mono" style={{ fontSize: 10, letterSpacing: '.18em', opacity: .6 }}>─── ETAPPER</div>
        </div>
        <div style={{ padding: '8px 22px' }}>
          {d.legs.map((s, i) => (
            <div key={i} style={{ display: 'grid', gridTemplateColumns: '56px 1fr', gap: 12, padding: '10px 0',
              borderBottom: i < d.legs.length - 1 ? '1px dashed rgba(26,31,26,.22)' : 'none' }}>
              <div>
                <div className="mono" style={{ fontSize: 11, letterSpacing: '.1em' }}>{s.t}</div>
                <div className="mono" style={{ fontSize: 9, opacity: .55, marginTop: 2 }}>{s.km}</div>
              </div>
              <div>
                <div style={{ fontSize: 16, lineHeight: 1.15 }}>{s.name}</div>
                <div className="hand" style={{ fontSize: 15, color: '#1a3a5a', marginTop: 2 }}>{s.note}</div>
              </div>
            </div>
          ))}
        </div>

        <section style={{ margin: '14px 18px 24px', padding: 14, background: '#fff8ea',
          border: '1px solid rgba(26,31,26,.2)', borderRadius: 4, position: 'relative' }}>
